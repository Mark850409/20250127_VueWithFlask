      ...
      <div v-for="comment in paginatedComments" 
        ...
        <div class="border-b bg-gray-50 p-4">
          <div class="flex items-center space-x-4">
            ...
            <div>
              <h3 class="font-medium text-lg text-gray-900">{{ comment.store_name }}</h3>
        ...
        <!-- 回覆列表 -->
        <div v-if="comment.replies && comment.replies.length" class="bg-gray-50 border-t">
          <div class="p-4 space-y-4">
            <div v-for="reply in comment.replies" :key="reply.id" class="flex items-start space-x-3">
              <img :src="shopAvatar" class="w-8 h-8 rounded-full mt-1">
              <div class="flex-1 bg-white rounded-lg p-4 shadow-sm">
                <div class="flex justify-between items-start mb-2">
                  <div>
                    <h4 class="font-medium text-gray-900">
                      <span class="bg-blue-100 text-blue-800 text-xs px-2 py-0.5 rounded-full mr-2">店家</span>
                      {{ reply.user_name }}
                    </h4>
  ...
  methods: {
    ...
    async getUserInfo(userId) {
      ...
      try {
        ...
        const userInfo = {
          userAvatar: `${import.meta.env.VITE_BACKEND_URL}/api/users/avatar/${userData.avatar.split('/').pop()}`,
